...
- Claude Code CLI 2.1.251: `-p/--print` non-interactive; `--output-format text|json|stream-json`; `--allowed-tools`/`--disallowed-tools`; `--append-system-prompt`; permission-mode flags; `--json-schema <schema>` (JSON Schema for structured output). SpecWitness-side schema validation + bounded retry is the mechanism (FR-14), and the CLI-side flag is deliberately unused — see the correction below.
  - **Correction, action item e2-B3, applied 2026-09-04 by story 6.2.** This line originally read *"No first-class JSON-Schema-constrained output flag observed"*. That was wrong for this binary and had been known to be wrong since Epic 2: story 2.4's author found `claude --help` on 2.1.251 listing `--json-schema` and recorded it in that story's Dev Agent Record (`docs/implementation-artifacts/epic-2-verification-contracts/2.4-claude-code-cli-adapter.md`), and the Epic 2 retrospective carried it as B3 (`docs/findings/epic-2-retro-2026-08-31.md`). **SpecWitness deliberately does not use the flag**, and the merged adapter shows it: the claude adapter's argv is the baseline `-p --output-format json` and nothing else (`src/providers/claude-code-cli.ts:55`), and it never reads `AgentPrompt.jsonSchema` — only the codex adapter does, via `--output-schema` (`src/providers/codex-cli.ts:671-675`). The reason is recorded in `src/domain/agent-provider.ts:89-91`: *steering a CLI with a schema is not validating against one*; a CLI-side constraint makes malformed output rarer, never impossible (ADR-001), so it can never replace the zod gate in `src/providers/invoke.ts` (AD-2). Nothing about the mechanism changes; only the capability record was stale. **This is a factual prose correction to a finalized planning artifact, made because story 6.2's spec names it in scope; no decision, FR or NFR is altered by it.**
