...
1. **TypeScript + Node.js, distributed as an npm package with a CLI** (`specwitness`, `npx`-runnable). Fits the target audience (JS-ecosystem agentic developers), the Playwright ecosystem, and the harness's subprocess invocation model. Node floor >=22.13 (**ADR-007**, raised from the >=22.12 originally recorded here: the pinned pnpm 11.24 requires >=22.13).
