...
**Narrow the fix to the nudge, not to reopening.** Reopening a `done` agent is sometimes exactly right: at 14:42:55Z arnold was reopened from `done` by a **peer message** — rambo had duplicated his `composePrompt` format and proposed moving it into the shared `text.ts` — and arnold answered with a written go-ahead two minutes later. That is the inbox gate working as designed on a finished story. The defect is specifically the **rebase nudge** reaching a merged agent with an instruction it must not follow; the mail must keep getting through.
