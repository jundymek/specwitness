...
export interface GenerateDraftInput {
   ...
   * FED: `src/cli/commands/contract.ts` passes the loaded `config.redaction` (the project's
   * `redaction.extraPatterns`) here. Until then nothing in the product built a
   * `RedactionOptions` from config at all, so this parameter was reachable only from tests
   * one level up as well. `tests/integration/authoring-extra-patterns.test.ts` now drives it
   * through the built binary and inspects the prompt the provider actually received.
   */
